...
class Shape(QtGui.QGraphicsItem):
        ...
        def  getNode (self, i):
                ...
                and (i < self.numOfNodes)):
                        m = Matrix (4,1);
                        m[0][0]= self.nodes[i].x;
                        m[1][0]= self.nodes[i].y;
                        m[2][0]= self.nodes[i].z;
                        m[3][0]= 1;
                        m = self.aff_matrix * m;
                        res = MatrixToPoint(m);
                        return res;        
                else :
                      raise IndexError("Shape.getNode (self, i):");  
        ...
        def paint(self, painter, option, widget):
                ...
                for side in self.sides:
                        ...
                        try:
                                ...
                                if( self.shadow):
                                        painter.setBrush(QtGui.QBrush(QtGui.QColor ( 0,0,0, (self.alphaShadow*self.alpha/255*100 /self.wall) )))
                                        painter.setPen(QtGui.QPen(QtGui.QColor ( 0, 0,0 , (self.alphaShadow*self.alpha/255*100 /self.wall )/12)))
