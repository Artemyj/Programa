...
class Side(QtGui.QGraphicsItem):
        ...
        def genShadowMat(self):
                ...
                if (self.paral  == 2):
                        ...
                        for node in self.nodes :
                                nd = Point3d()
                                nd.x = ( - node.x*self.lamp.z + (self.wall  - node.z)*self.lamp.x )/(node.z + self.wall - self.lamp.z)
                                nd.y = (- node.y*self.lamp.z + (self.wall  - node.z)*self.lamp.y) /(node.z + self.wall - self.lamp.z  )
                                nd.z = 0
                                self.shadNodes.append(nd)
        ...
        def paint(self, painter, option, widget):
               ...
               if self.showNormals:
                        ...
                        painter. drawLine(( self.center() - self.normalVector).toQPointF(), ((self.center() - self.normalVector) * 3).toQPointF() )
